...
Explain why Memory Engine is intentionally local-first and how that affects deployment, operations, and risk.
...
- Reproducibility comes from explicit scripts and bounded runtime contracts.

## In-Class Flow (35-50 min)

1. Draw the process map (`proxy`, `api`, `worker`, `beat`, `db`, `redis`, `minio`).
2. For each process, identify one failure and one detection path.
3. Discuss what changes if services move off-node.

## Reflection Prompts

- Which local assumptions are load-bearing for trust?
- Which defaults are safe for class use but unsafe for public deployment?
