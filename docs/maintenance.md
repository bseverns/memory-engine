...
- `ENGINE_DEPLOYMENT` declares the active deployment kind (`memory` default; planned: `question`, `prompt`, `repair`, `witness`, `oracle`) so `/ops/` and participant framing can branch safely without changing routes.
